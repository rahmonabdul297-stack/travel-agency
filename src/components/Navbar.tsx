...
import {
  FaSun, FaMoon, FaWhatsapp, FaPhone, FaArrowRight,
  HiMenuAlt3, HiX, FaPlane,
} from '@/lib/icons';
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <Link to="/" className="flex items-center gap-2 group">
              <motion.div
                ...
                className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl bg-brand-gradient flex items-center justify-center shadow-brand-glow"
              >
                <FaPlane className="text-white text-lg lg:text-xl" />
              </motion.div>
              <div className="flex flex-col leading-tight">
                <span className={`font-display font-bold text-sm lg:text-lg ${theme === 'dark' ? 'text-ink-dark-primary' : 'text-ink-light-primary'}`}>
                  Excellent Travel
                </span>
                <span className="text-[10px] lg:text-xs bg-brand-gradient bg-clip-text text-transparent font-semibold">
                  {settings?.tagline?.split(' ').slice(0, 4).join(' ') || 'A Reliable Way'}
                </span>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-2 lg:gap-3">
              ...
              {settings?.phone && (
                <a
                  ...
                  style={{ color: theme === 'dark' ? 'var(--tw-color-ink-dark-secondary)' : undefined }}
                >
              ...
              {settings?.whatsapp && (
                <a
                  ...
                  className="hidden xl:flex w-10 h-10 rounded-xl bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white items-center justify-center transition-colors duration-300"
                >
                  <FaWhatsapp />
                </a>
              )}

              <Link to="/contact" className="hidden sm:flex btn-brand text-sm items-center gap-2">
              ...
              <button
                ...
                className={`lg:hidden w-10 h-10 rounded-xl flex items-center justify-center ${
                  theme === 'dark' ? 'bg-surface-dark-hover text-ink-dark-primary' : 'bg-gray-100 text-ink-light-primary'
                }`}
              >
          ...
          <>
            ...
            <motion.div
              ...
              className={`fixed top-0 right-0 bottom-0 w-80 max-w-[85vw] z-50 lg:hidden overflow-y-auto ${
                theme === 'dark' ? 'bg-surface-dark-card border-l border-border-dark' : 'bg-white border-l border-border-light'
              }`}
            >
              <div className="p-6 pt-20">
                <div className="flex flex-col gap-2">
                  ...
                  <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 }}>
                    <NavLink
                      to="/services/flight-tickets"
                      className={({ isActive }) =>
                        `block px-4 py-3 rounded-xl font-medium transition-colors ${
                ...
                >
                  {settings?.phone && (
                    <a href={`tel:${settings.phone}`} className="flex items-center gap-3 text-sm">
                  ...
                  <Link to="/contact" className="btn-brand w-full text-center mt-4">
                    Quick Inquiry
                  </Link>
                </motion.div>
              </div>
